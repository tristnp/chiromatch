...
function validateCitySeoRecord(record) {
  ...
  const normalized = {
    ...
    population: Number.isFinite(record.population) ? Number(record.population) : 0,
    populationRank: Number.isFinite(record.populationRank) ? Number(record.populationRank) : 0,
  ...
  if (!normalized.population) {
    issues.push("missing population");
  }

  if (!normalized.populationRank) {
    issues.push("missing populationRank");
  }
